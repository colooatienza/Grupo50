...
<body>
 ...
 for ($i = 1; $i <= 19; $i++) {
   $evalua_campos[$i]="\"registro_ant > color_gris\"";
	 ...
	 $foto_vieja = isset($_POST['fotoo'])? $_POST['fotoo']:""; 
	 ...
	 if($clave=="Clave"){
	  ...
	  $valido_todo=true;
		 	if($imagen_vieja=='si'){ $valido_todo=false; $imagen_vieja="habilitada";}
     ...
     $cons_mail=("select * from usuarios where BINARY mail='".utf8_decode($email)."' and 
...
$i=$i+strlen($dominio)+1;
   ...
   //verifico que solo sea formato para imagen y no OTRA COSA:::		
	...
	<form method="post" name="devuelve" id="devuelve" action="modificarUsuario1.php"   ENCTYPE="multipart/form-data" >
   ...
   <input type="hidden" name="imagen_vieja" value="<?php echo $imagen_vieja;?>" >
 ...
 // }
			$usuario_viejoo=$_SESSION['usuario'];
				
						
						$sql3 = ("UPDATE usuarios set nombredeusuario='$usuario', clave='$clave', fechadenacimiento='$fecha',nacionalidad='$nacionalidad',sexo='$sexo',  mail='$email',telefono='$telefono',celular='$telefono_movil',descripcion='$descripcion',foto='$destino'   where nombredeusuario='$usuario_viejoo'");
			...
			?>
			<form method="post" name="regii" id="regii" action="registrarUsuario3.php?tipo=2"   ENCTYPE="multipart/form-data" >
